...
def prepare_inputs_and_targets(data_dict, device=torch.device("cuda"), v=3, timespan=2.0, from_list=False, args=None):
    ...
    if from_list is True:
        ...
        if type(data_dict[0]['global_frame']) is bool:
            global_frame = data_dict[0]['global_frame']
        else:
            global_frame = data_dict[0]['global_frame'].flatten()[0].item()
        if global_frame:
            return [prepare_inputs_and_targets(ddi, device=device, v=v, timespan=timespan * N, args=args) for ddi in data_dict]
        else:
            return [prepare_inputs_and_targets(ddi, device=device, v=v, timespan=timespan, args=args) for ddi in data_dict]
    ...
    input_dict = {
        ...
        "context_camtoworlds_global": data_dict['context']['camtoworld_global'].reshape(b, context_t, v, 4, 4),
        ...
        "target_camtoworlds_global": data_dict["target"]["camtoworld_global"].reshape(b, target_t, v, 4, 4),
    ...
    if "instances_pose" in data_dict["context"]:
        input_dict['context_instances_pose'] = data_dict["context"]['instances_pose'].reshape(
            b, context_t, args.num_bbox, 4, 4
        )

        input_dict['context_instances_corner'] = data_dict['context']['instances_corner'].reshape(
            b, context_t, args.num_bbox, 8, 3
        )
        input_dict['context_instances_corner_local'] = data_dict['context']['instances_corner_local'].reshape(
            b, context_t, args.num_bbox, 8, 3
        )
    ...
    if "instances_pose" in data_dict["target"]:
        input_dict['target_instances_pose'] = data_dict["target"]['instances_pose'].reshape(
            b, target_t, args.num_bbox, 4, 4
        )

        input_dict['target_instances_corner'] = data_dict['target']['instances_corner'].reshape(
            b, target_t, args.num_bbox, 8, 3
        )
        input_dict['target_instances_corner_local'] = data_dict['target']['instances_corner_local'].reshape(
            b, target_t, args.num_bbox, 8, 3
        )
