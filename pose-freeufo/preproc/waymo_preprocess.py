...
from ufo.visualization.visualization_tools import depth_visualizer, scene_flow_to_rgb
...
class WaymoProcessor:
    def __init__(
        ...
        process_keys=["images", "lidar", "calib", "pose", "ground", "dynamic_masks", "objects", "videos"],
    ...
    def convert_one(self, file_id):
        ...
        if "objects" in self.process_keys:
            ...
            object_info_dir = f"{self.save_dir}/{str(scene_id).zfill(3)}/instances"
            if not os.path.exists(object_info_dir):
                os.makedirs(object_info_dir)
        ...
        # save a video
        if "videos" in self.process_keys:
    ...
    def make_json(self, file_id):
        ...
        scene_dict["frame_instances"] = json.load(open(f"{file_folder}/instances/frame_instances.json"))
        scene_dict["instances_info"] = json.load(open(f"{file_folder}/instances/instances_info.json"))
            ...
            for l in frame.laser_labels:
                ...
                if str_id not in instances_info:
                    instances_info[str_id] = dict(
                        ...
                        num_lidar=l.num_lidar_points_in_box,
